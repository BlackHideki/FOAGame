...
public class GameOverScene implements Scene {
	...
	public void action() {
		if (!scoreCheckFlg) {
			ranking.getText().add(String.format("%.1f", score));
			float[] tmp = new float[ranking.getText().size()];
			...
			for (int i = 0; i < tmp.length - 1; i++) {
				ranking.getText().add(String.format("%.1f", tmp[i]));
			}
	...
	public void paint(Graphics graphics) {
		...
		graphics.drawString("SCORE : " + String.format("%.1f", score), Execute.WINDOW_WIDTH / 2 - 150, Execute.WINDOW_HEIGHT / 2 + menuRetry.getSize().height * 3);
	}
